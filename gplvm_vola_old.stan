functions{
    ...
    matrix kernel_f(vector[] X1, vector[] X2, real kernel_l, vector Sigma, int kernel, vector noise_std, real jitter){
        int N = size(X1);
        matrix[N,N] K;
...
transformed parameters{
    ...
    {matrix[N,N] K[D];
    for (d in 1:D){
        K[d] = kernel_f(X, X, kernel_l, Sigma[:,d], kernel, rep_vector(0,N), jitter);
...
generated quantities{
    ...
    for (d in 1:D){
        //matrix[N,D] residue;
        K[d] = kernel_f(X, X, kernel_l, Sigma[:,d], kernel, rep_vector(0,N), jitter);
        // K_noise[d] = kernel_f(X, X, kernel_l, Sigma[:,d], kernel, noise_std, jitter);
